...
type HTTPPool struct {
	...
	httpGetter map[string]*httpGetter // keyed by e.g. "http://10.0.0.2:8080"
}
...
func (h *httpGetter) Get(group, key string) ([]byte, error) {
	...
	if res.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("HTTPGet: %d %s", res.StatusCode, res.Status)
	}

	bytes, err := ioutil.ReadAll(res.Body)
	if err != nil {
		return nil, fmt.Errorf("HTTPGet: ReadAll: %v", err)
	}
